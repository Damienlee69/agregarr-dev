...
describe('normalizeSortTitleArticle', () => {
  it('strips a leading "The"/"A"/"An" by default', () => {
    expect(normalizeSortTitleArticle('The Accountant')).toBe('Accountant');
    expect(normalizeSortTitleArticle('A Beautiful Mind')).toBe(
      'Beautiful Mind'
    );
    expect(normalizeSortTitleArticle('An American Werewolf in London')).toBe(
      'American Werewolf in London'
    );
  });

  it('moves the article to the end with a comma in "moveToEnd" mode', () => {
    expect(normalizeSortTitleArticle('The Accountant', 'moveToEnd')).toBe(
      'Accountant, The'
    );
  });

  it('leaves the title untouched in "off" mode', () => {
    expect(normalizeSortTitleArticle('The Accountant', 'off')).toBe(
      'The Accountant'
    );
  });

  it('leaves titles with no leading article untouched, regardless of mode', () => {
    expect(normalizeSortTitleArticle('The Chronicles of Riddick')).toBe(
      // "The" is stripped here too - only the leading word matters, not
      // whether the rest of the title also contains an article-like word
      'Chronicles of Riddick'
    );
    expect(normalizeSortTitleArticle('Terminator 2')).toBe('Terminator 2');
  });

  it('requires a space after the article - "Terminator" is not mistaken for starting with "A"', () => {
    expect(normalizeSortTitleArticle('Amelie')).toBe('Amelie');
    expect(normalizeSortTitleArticle('Anaconda')).toBe('Anaconda');
  });

  it('is case-insensitive when matching the article but preserves the original casing in the result', () => {
    expect(normalizeSortTitleArticle('the accountant')).toBe('accountant');
    expect(normalizeSortTitleArticle('THE ACCOUNTANT')).toBe('ACCOUNTANT');
  });
});
...
describe('stripCollectionSuffix', () => {
  it('strips a trailing " Collection" when enabled', () => {
    expect(stripCollectionSuffix('The Accountant Collection', true)).toBe(
      'The Accountant'
    );
  });

  it('leaves the title untouched when disabled', () => {
    expect(stripCollectionSuffix('The Accountant Collection', false)).toBe(
      'The Accountant Collection'
    );
  });

  it('leaves titles with no "Collection" suffix untouched', () => {
    expect(stripCollectionSuffix('The Chronicles of Riddick', true)).toBe(
      'The Chronicles of Riddick'
    );
  });

  it('does not strip a mid-word match like "Collections" or a name that merely contains "Collection"', () => {
    expect(stripCollectionSuffix('Auto Genre Collections', true)).toBe(
      'Auto Genre Collections'
    );
    expect(stripCollectionSuffix('Collection of Shorts', true)).toBe(
      'Collection of Shorts'
    );
  });

  it('never produces an empty title - a collection literally named "Collection" is left alone', () => {
    expect(stripCollectionSuffix('Collection', true)).toBe('Collection');
  });
});

describe('isAgregarrOwnedSortTitle', () => {
  it('owns an absent, natural, or leftover positional sort title', () => {
    expect(isAgregarrOwnedSortTitle(undefined, 'Cameras')).toBe(true);
    expect(isAgregarrOwnedSortTitle('', 'Cameras')).toBe(true);
    expect(isAgregarrOwnedSortTitle('Cameras', 'Cameras')).toBe(true);
    // Left behind by a demotion - has to be cleaned up or Plex keeps showing
    // the collection in its promoted section.
    expect(isAgregarrOwnedSortTitle('!003_Cameras', 'Cameras')).toBe(true);
  });

  it('owns a title it recorded normalizing', () => {
    // The marker is what lets switching to 'off' restore "The Godfather"
    // instead of reading Agregarr's own earlier output as a manual edit.
    expect(isAgregarrOwnedSortTitle('Godfather', 'The Godfather', true)).toBe(
      true
    );
  });

  it('owns the leftover value from a Sort Title override it has cleared', () => {
    // Agregarr wrote "ZZZ_Video Games" itself, through an override that has
    // since been cleared. Nothing records what the override produced, so the
    // value is indistinguishable from a human edit - but clearing the
    // override is exactly the request to stop using it, and without this the
    // collection stays pinned to it forever.
    expect(
      isAgregarrOwnedSortTitle(
        'ZZZ_Video Games',
        'Video Games',
  ...
  it('does not own a sort title the user typed in Plex', () => {
    // "...Cameras" filed under C because the user set "Cameras" in Plex.
    // No article is involved - the damage came from the write replacing
    // whatever was there with the name.
    // A value that matches nothing Agregarr would produce for this name.
    expect(isAgregarrOwnedSortTitle('Alien 1', 'Alien Collection')).toBe(false);
    expect(isAgregarrOwnedSortTitle('ZZ_Last', 'Some Collection')).toBe(false);
    // Still hands-off on a collection Agregarr has never managed, which is
    // what separates it from the cleared-override case above.
    expect(
      isAgregarrOwnedSortTitle('Alien 1', 'Alien Collection', undefined, false)
    ).toBe(false);
  });

  it('owns a value Plex produced by stripping leading punctuation', () => {
    // Plex drops leading punctuation as well as the article when it generates
    // a sort title, and normalizeSortTitleArticle now does the same - so
    // "Cameras" on "... Cameras" is recognised as matching Agregarr's own
    // output rather than read as a human's edit. Under strip and moveToEnd
    // that makes the write a no-op; 'off' restores the punctuation.
    expect(isAgregarrOwnedSortTitle('Cameras', '... Cameras')).toBe(true);
    expect(
      isAgregarrOwnedSortTitle(
        'Caller, The Call of the Wild, and The Call',
        '...The Caller, The Call of the Wild, and The Call'
      )
    ).toBe(true);
  });

  it('owns a sort title Plex generated by stripping the article itself', () => {
    // Plex writes its own sortTitle for a new collection with the leading
    // article removed, so "The Avengers" arrives sorting as "Avengers" with
    // no marker and nothing of Agregarr's involved. Treating that as a human
    // edit would make 'moveToEnd' and 'off' silently do nothing to it.
    expect(isAgregarrOwnedSortTitle('Avengers', 'The Avengers')).toBe(true);
    expect(isAgregarrOwnedSortTitle('Godfather', 'The Godfather')).toBe(true);
  });
});
...
describe('leading articles under a shared multi-collection prefix', () => {
  // Collections generated by one multi-collection config all sit behind the
  // same prefix, so the name after it is the only thing ordering them. An
  // un-normalized leading article files that sibling under "The" and drops
  // it out of alphabetical order relative to the rest of its own group.
  const group = (names: string[], mode: 'strip' | 'moveToEnd' | 'off') =>
    names
      .map((n) =>
        buildSortTitleFromOverride(
          '!010_',
          normalizeSortTitleArticle(n, mode),
          true
        )
      )
      .sort();

  it('keeps the group alphabetical when articles are stripped', () => {
    expect(group(['Babysitter', 'The Crow', 'Defender'], 'strip')).toEqual([
      '!010_Babysitter',
      '!010_Crow',
      '!010_Defender',
    ]);
  ...
  it('orders by the remaining title when articles move to the end', () => {
    expect(group(['Babysitter', 'The Crow', 'Defender'], 'moveToEnd')).toEqual([
      '!010_Babysitter',
      '!010_Crow, The',
      '!010_Defender',
    ]);
  ...
  it('leaves the article in place - and out of order - when handling is off', () => {
    // The behaviour the fix exists to avoid, pinned so it is a deliberate
    // consequence of 'off' rather than something that silently regresses.
    expect(group(['Babysitter', 'The Crow', 'Defender'], 'off')).toEqual([
      '!010_Babysitter',
      '!010_Defender',
      '!010_The Crow',
    ]);
...
describe('collection suffix mode', () => {
  it('adds the suffix only where it is missing', () => {
    expect(addCollectionSuffix('Alien', true)).toBe('Alien Collection');
    expect(addCollectionSuffix('Alien Collection', true)).toBe(
      'Alien Collection'
    );
  });

  it('is a no-op when disabled', () => {
    expect(addCollectionSuffix('Alien', false)).toBe('Alien');
  });

  it('round-trips with the strip direction', () => {
    const added = addCollectionSuffix('The Matrix', true);
    expect(added).toBe('The Matrix Collection');
    expect(stripCollectionSuffix(added, true)).toBe('The Matrix');
  });

  it('leaves a blank name alone rather than inventing one', () => {
    expect(addCollectionSuffix('', true)).toBe('');
    expect(addCollectionSuffix('   ', true)).toBe('   ');
  });

  it('defaults to leaving the name alone', () => {
    // The default has to be inert: anything else renames every TMDb franchise
    // collection on the first sync after an upgrade, with no one asking.
    expect(resolveCollectionSuffixMode(undefined)).toBe('leave');
  });

  it('takes the collection at its word otherwise', () => {
    expect(resolveCollectionSuffixMode('add')).toBe('add');
    expect(resolveCollectionSuffixMode('leave')).toBe('leave');
    expect(resolveCollectionSuffixMode('strip')).toBe('strip');
  });
});
