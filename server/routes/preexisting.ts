...
/**
 * POST /api/v1/preexisting/:id/sync
 * Apply this collection's pending changes to Plex now, rather than waiting
 * for the next full sync.
 *
 * Mirrors POST /collections/:id/sync. Pre-existing collections had no
 * equivalent: their suffix, sort title and rank were only ever written by
 * the full sync's sortTitle pass, so changing any of them left the row
 * showing "Needs Sync" for however long that took - beside an icon that
 * looks exactly like the working one on an Agregarr collection.
 */
preExistingRoutes.post('/:id/sync', isAuthenticated(), async (req, res) => {
  const { id } = req.params;

  try {
    const config = preExistingCollectionConfigService
      .getConfigs()
      .find((c) => c.id === id);
    ...
    // Same guard the collections route uses: the full sync walks every
    // config and would race this one writing the same collection.
    ...
    if (collectionsSync.running) {
      return res.status(409).json({
        ...
        message:
          'Cannot sync an individual collection while a full sync is running. Please wait for the full sync to complete.',
      });
    }
    ...
    await new HubSyncService().syncPreExistingCollectionSortTitles(
      plexClient,
      id
    );

    // Clear the pending flag, or the row keeps showing the red "needs sync"
    // arrow it was just synced from - the same bookkeeping the full sync
    // does for every config it walks.
    settings.markCollectionSynced(id, 'preExisting');

    const updated = preExistingCollectionConfigService
      .getConfigs()
      .find((c) => c.id === id);

    logger.info(
      `Individual sync completed for pre-existing collection ${config.name}`,
      { label: 'Pre-existing Collections API', collectionId: id }
    );

    return res.status(200).json({
      preExistingCollectionConfig: updated,
      message: 'Collection synced successfully',
    });
  } catch (error) {
    logger.error('Failed to sync individual pre-existing collection', {
      label: 'Pre-existing Collections API',
      error: error instanceof Error ? error.message : String(error),
      configId: id,
    });
