...
const messages = defineMessages({
  ...
  collectionSuffixHelp:
    'Choose to add, remove or leave the " Collection" suffix on the collection. This renames the collection in Plex and does not affect its sort title. Default: Leave as is. Use Bulk Edit to adjust many collections at once.',
...
function plexSetSortTitle(target: SortTitleConfigLike): string | undefined {
  ...
  const sortKey = sortKeyForName(
    target.name || '',
    target.collectionSuffixMode
  );
...
function computeAgregarrSortTitle(target: SortTitleConfigLike): string {
  ...
  // Same key the server sorts by: a suffix Agregarr added is decoration on
  // the title and stays out of it.
  const sortKey = sortKeyForName(name, target.collectionSuffixMode);
      ...
      <Formik
        initialValues={{
          ...
          collectionSuffixMode:
            (config as PreExistingCollectionConfig).collectionSuffixMode ??
            'leave',
        ...
        onSubmit={async (values, { setFieldError }) => {
          ...
          const configToSave: CollectionFormConfig = {
            ...
            !values.sortTitleOverride?.trim()
              ? { sortTitleReset: true }
              : {}),
            collectionSuffixMode: values.collectionSuffixMode,
                          ...
                          {isPreExisting && (
                            <div className="form-row">
                              ...
                              <div className="form-input-area">
                                ...
                                {/* Collection Suffix - a real Plex rename,
                                    decided per collection because nothing in
                                    a config tells a TMDb franchise apart from
                                    a network. */}
                                {isPreExisting && (
                                  <div className="form-row">
                                    <label
                                      htmlFor="collectionSuffixMode"
                                      className="text-label"
                                    >
                                      {intl.formatMessage(
                                        messages.collectionSuffix
                                      )}
                                    </label>
                                    <div className="form-input-area">
                                      <Field
                                        as="select"
                                        id="collectionSuffixMode"
                                        name="collectionSuffixMode"
                                        className="block w-full rounded-md border border-stone-600 bg-stone-800 px-3 py-2 text-sm text-white focus:border-orange-500 focus:outline-none"
                                      >
                                        <option value="leave">
                                          {intl.formatMessage(
                                            messages.collectionSuffixLeave
                                          )}
                                        </option>
                                        <option value="strip">
                                          {intl.formatMessage(
                                            messages.collectionSuffixStrip
                                          )}
                                        </option>
                                        <option value="add">
                                          {intl.formatMessage(
                                            messages.collectionSuffixAdd
                                          )}
                                        </option>
                                      </Field>
                                      <div className="label-tip mt-1">
                                        {intl.formatMessage(
                                          messages.collectionSuffixHelp
                                        )}
                                      </div>
                                    </div>
                                  </div>
